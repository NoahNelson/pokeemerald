...
static u8 HandleReplaceSector(u16 sectorId, const struct SaveSectorLocation *locations)
{
    ...
    {
        *(SRAM_BASE + sector * SECTOR_SIZE + SECTOR_SIGNATURE_OFFSET + 1 + i) = ((u8 *)gReadWriteSector)[i + 1 + SECTOR_SIGNATURE_OFFSET];
    }
...
static u8 WriteSectorSignatureByte_NoOffset(u16 sectorId, const struct SaveSectorLocation *locations)
{
    ...
    *(SRAM_BASE + sector * SECTOR_SIZE + SECTOR_SIGNATURE_OFFSET) = SECTOR_SIGNATURE & 0xFF;
    return SAVE_STATUS_OK;
}
...
static u8 CopySectorSignatureByte(u16 sectorId, const struct SaveSectorLocation *locations)
{
    ...
    *(SRAM_BASE + sector * SECTOR_SIZE + SECTOR_SIGNATURE_OFFSET) = ((u8 *)gReadWriteSector)[SECTOR_SIGNATURE_OFFSET];
    return SAVE_STATUS_OK;
}
